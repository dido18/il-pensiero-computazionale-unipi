...
![width:50px height:50px](massima.jpg) <!-- Setting width to 50px -->
...
> Dato un vettore $D[1 : n]$ formato da $n$ numeri positivi e negativi, si vuole determinare la porzione $D[a:v]$ > avente somma  $\sum_i=a^v$ massima.
> 

Vedremo tre possibili soluzioni:
1. Algoritmo cubico $O(n^3)$
2. Algoritmo quadratico  $O(n^2)$
3. Algoritmo lineare*  $O(n)$
